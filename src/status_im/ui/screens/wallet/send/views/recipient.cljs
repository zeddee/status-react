...
(defn update-recipient [web3 chain transaction error-message value]
  (if (ens/is-valid-eth-name? value)
    (do (ens/get-addr web3
         ...
         [react/view styles/centered
          (when @error-message
            [tooltip/tooltip @error-message {:color        colors/white
                                             ...
                                             :bottom-value 15}])
          [react/text-input
           {:on-change-text         (partial update-recipient web3 chain transaction error-message)
            :auto-focus             true
